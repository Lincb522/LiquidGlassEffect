...
fragment half4 liquidGlassEffect(VertexOutput input [[stage_in]],
    ...
    if (shapeDistance < 0.005f) {
        ...
        if (edgeShiftFactor <= 0.0f) {
            // Apply UV mapping for shared textures
            float2 mappedUV = float2(input.uv) * uniforms.uvScale + uniforms.uvOffset;
            outputColor = background.sample(textureSampler, mappedUV);
            ...
            // Apply UV mapping for shared textures
            float2 baseUv = float2(input.uv) * uniforms.uvScale + uniforms.uvOffset;
            // Note: offsetUv is in local UV space, we need to scale it too if we want perfect physics, 
            // but for visual approximation, applying it directly to the mapped UV is often enough.
            // A more correct way would be: mappedUV + offsetUv * uniforms.uvScale
            // Let's stick to simple addition first as offsetUv is small.
            
            half4 refractedWithDispersion = sampleWithDispersion(background, baseUv, float2(offsetUv) * uniforms.uvScale, uniforms.dispersionStrength);
